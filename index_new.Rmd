...
output$groupvar <- renderUI({
    ...
    varSelectInput(inputId = 'group_var',
                   label = 'Choose categorical grouping (x) variable (in the case of one-sample tests, the variable will have one level ony)',
                   selected = NULL,
                   data = df_categorical())
    })
        ...
        if(length(levels(df_analysis()$x_variable)) == 1){
            ...
            if(input$test_type == 'parametric'){
                ...
                t_test(y_variable ~ 1,
                       data = df_analysis(),
                       mu = input$mu_value) %>% 
                    mutate(test = 't-test (one-sample)',
                           `true mean` = input$mu_value) %>% 
                    select(test, `true mean`, statistic, df, p) %>% 
                    mutate(p = round(p, 5)) %>% 
                    setNames(nm = c('Test', 'True mean', 'Statistic (t)', 'DF', 'p-value'))
            ...
            if(input$test_type == 'parametric'){
                ...
                df_analysis() %>% 
                    ...
                    setNames(nm = c('Test', 'Alternative', 'Group 1', 'Group 2', 
                                    'Group 1: mean', 'Group 2: mean', 'Delta means', 
                ...
                df_analysis() %>% 
                    wilcox_test(y_variable ~ x_variables) %>% 
                    mutate(test = 'Mann-Whitney U test (with continuity correction) (two-sample)') %>% 
                    select(test, group1, group2, statistic, p) %>% 
                    mutate(p = round(p, 5)) %>% 
                    setNames(nm = c('Test', 'Group 1', 'Group 2', 'Statistic (W)', 'p-value'))
            ...
            if(input$test_type == 'parametric'){
                ...
                df_analysis() %>% 
                    kruskal_test(y_variable ~ x_variable) 
