...
Please cite this app as: Kamerman PR (2022). statr: online statistical tests. Version: v0.1.3. DOI: [10.5281/zenodo.5977332](https://doi.org/10.5281/zenodo.5977332).
...
checkboxInput(inputId = 'effect_size',
              label = 'Calculate effect size (Note: bais-corrected and accelerated bootstrapped 95% confidence intervals, with 1000 replications, so calculation can take a while to perform...be patient)',
              value = FALSE)
...
radioButtons(inputId = 'tab_test',
             ...
             choices = c("Pearson's chi-squared test (or Fisher's exact test if any expected values < 5)" = 'chi',
                         ...
                         'CMH test (only the row variable is ordinal)' = 'cmh_y',
                         'CMH test (only the column variable is ordinal)' = 'cmh_x'))
```
...
### Mosaic plot (2-way tables only, set ID column to 'None')
...
output$tabresult <- renderTable({
    ...
    if(input$column_tab != input$row_tab){
        ...
        if(input$tab_test == 'chi' & test == 0){
            ...
            cmh.cor <- CMHtest(t(table(df_xtab())),
                               types = 'cor') %>% 
                .$table %>% 
                as_tibble() %>% 
