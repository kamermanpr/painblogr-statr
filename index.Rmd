...
To plot your data, please try the companion app, [`plotr`](https://kamermanpr.shinyapps.io/painblogr-plotr/){ target='_blank' }

**To start, choose the test you want from the drop-down menu labelled _“Select a test”_.**
...
- See examples of the data structure  <a href="https://drive.google.com/file/d/16-bbwhByyNzdjVkebKd_ePiTCjggpWR6/view?usp=sharing" target="_blank">here</a>
...
df <- reactive({
    ...
    if(input$type0 == TRUE & input$header0 == TRUE) {
        read_csv(input$file0$datapath,
                 col_names = input$header0) %>% 
        clean_names()
      
    } else if(input$type0 == TRUE & input$header0 == FALSE){
        read_csv(input$file0$datapath,
                 col_names = input$header0) %>% 
        setNames(nm = c('X1', 'X2'))
      
    } else if(input$type0 == FALSE & input$header0 == TRUE) {
...
- See examples of the data structure <a href="https://drive.google.com/file/d/1u1Km0zyTz4Ld385En6zNLUIZPYUf_oMF/view?usp=sharing" target="_blank">here</a>
...
aov_data <- reactive({
    ...
    if(input$type1 == TRUE & input$header1 == TRUE) {
        read_csv(input$file1$datapath,
                 col_names = input$header1) %>% 
        clean_names()
      
    } else if(input$type1 == TRUE & input$header1 == FALSE){
        n <- dim(input$file1)[2]
        read_csv(input$file1$datapath,
                 col_names = input$header1) %>% 
        setNames(nm = LETTERS[1:n])
      
    } else if(input$type1 == FALSE & input$header1 == TRUE) {
...
- See examples of the data structure  <a href="https://drive.google.com/file/d/1-_7rD0M29lnRzzmFFvKeHJqGBcq9_FNG/view?usp=sharing" target="_blank">here</a>
...
cor_data <- reactive({
    ...
    if(input$type2 == TRUE & input$header2 == TRUE) {
        read_csv(input$file2$datapath,
                 col_names = input$header2) %>% 
        clean_names()
      
    } else if(input$type2 == TRUE & input$header2 == FALSE){
        read_csv(input$file2$datapath,
                 col_names = input$header2) %>% 
        setNames(nm = c('X', 'Y'))
      
    } else if(input$type2 == FALSE & input$header2 == TRUE) {
...
renderPrint({
    ...
    corr <- cor_data() %>% 
        filter(complete.cases(.)) %>% 
        set_names(c('column_1.x', 'column_2.y'))
    
    if(input$test2 == "Pearson's correlation") {
        with(corr, cor.test(x = column_1.x, 
                            y = column_2.y, 
                            method = 'pearson'))
    } else {
        with(corr, cor.test(x = column_1.x, 
...
- Missing data pairs will be removed.  
- See examples of the data structure  <a href="https://drive.google.com/file/d/1ADvtOlIND9d_YTFBgN_0jPQpKiubSwfD/view?usp=sharing" target="_blank">here</a> 
...
lm_data <- reactive({
    ...
    if(input$type3 == TRUE & input$header3 == TRUE) {
        read_csv(input$file3$datapath,
                 col_names = input$header3) %>% 
        clean_names()
      
    } else if(input$type3 == TRUE & input$header3 == FALSE){
        read_csv(input$file3$datapath,
                 col_names = input$header3) %>% 
        setNames(nm = c('X', 'Y'))
      
    } else if(input$type3 == FALSE & input$header3 == TRUE) {
...
renderPrint({
    ...
    summary(lm(column_2.y ~ column_1.x, data = ols))
})
```
...
- Missing data pairs will be removed.  
- See examples of the data structure  <a href="https://drive.google.com/file/d/1HrvFwDZx0BAJmMWgcrKNoMUoyH64P3SW/view?usp=sharing" target="_blank">here</a>
